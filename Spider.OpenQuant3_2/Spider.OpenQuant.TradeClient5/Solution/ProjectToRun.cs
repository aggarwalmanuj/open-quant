...
namespace Spider.OpenQuant.TradeClient5.Solution
{
    public class ProjectToRun
    {
        ...
        public static ProjectToRun QuoteServerProject()
        {
            return GenerateProject(ProjectTemplateType.QuoteServer);
        }
        ...
        public ProjectToRun WithProjectIndex(int index)
        {
            ...
            if (ProjectTemplate == ProjectTemplateType.Close)
                OQProjectName = Enum.Parse(typeof (CloseOrderProjectNames), "Close_" + index.ToString("000")).ToString();
            else if (ProjectTemplate == ProjectTemplateType.OpenLong)
                OQProjectName = Enum.Parse(typeof (OpenOrderProjectNames), "Open_" + index.ToString("000")).ToString();
            else if (ProjectTemplate == ProjectTemplateType.OpenShort)
                OQProjectName = Enum.Parse(typeof (OpenOrderProjectNames), "Open_" + index.ToString("000")).ToString();
            else if (ProjectTemplate == ProjectTemplateType.ReverseOpen)
                OQProjectName = Enum.Parse(typeof(ReverseOpenOrderProjectNames), "ReverseOpen_" + index.ToString("000")).ToString();
            else if (ProjectTemplate == ProjectTemplateType.ReverseClose)
                OQProjectName = Enum.Parse(typeof(ReverseCloseOrderProjectNames), "ReverseClose_" + index.ToString("000")).ToString();
            else if (ProjectTemplate == ProjectTemplateType.QuoteServer)
                OQProjectName = "QuoteServer_"+ index.ToString("000").ToString(CultureInfo.InvariantCulture);;
